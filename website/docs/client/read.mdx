...
To read data from database with rex, a query object need to be created first. Rex generates query object for each model in schema. For example, `client.QueryUser()` returns a query object for users table.
